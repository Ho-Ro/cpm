...
static void dumptrace( z80info *z80 ) {
    printf( "a%.2X f%.2X bc%.4X de%.4X hl%.4X ", A, F, BC, DE, HL );
    printf( "ix%.4X iy%.4X sp%.4X pc%.4X:%.2X  ", IX, IY, SP, PC, z80->mem[ PC ] );
    disassem( z80, PC, stdout );
    printf( "\r\n" );

    if ( logfile ) {
        fprintf( logfile, "a%.2X f%.2X bc%.4X de%.4X hl%.4X ", A, F, BC, DE, HL );
        fprintf( logfile, "ix%.4X iy%.4X sp%.4X pc%.4X:%.2X  ", IX, IY, SP, PC, z80->mem[ PC ] );
...
static void suffix( char *str, const char *suff ) {
    while ( *str != '\0' && *str != '.' )
        str++;
...
boolean input( z80info *z80, byte haddr, byte laddr, byte *val ) {
    ...
    case 0x00:
        ...
        again:
            ...
            while ( !WaitNextEvent( keyDownMask | autoKeyMask, &ev, 20, nil ) )
                ;

            data = ev.message & charCodeMask;

            if ( ( data == '.' && ( ev.modifiers & cmdKey ) ) || data == INTR_CHAR ) {
                command( z80 );
                goto again;
            ...
            while ( ( data > 0x7f && errno == EINTR ) || data == INTR_CHAR ) {
                command( z80 );
                data = kget( 0 );
                /* data = getchar(); */
            }
    ...
    {
        EventRecord ev;
        *val = EventAvail( keyDownMask | autoKeyMask, &ev ) ? 0xFF : 0;
    }
#elif defined DJGPP
        *val = ( kbhit() ) ? 0xFF : 0;
#else /* UNIX or BeBox */
...
word read_mem( z80info *z80, word addr ) {
#ifdef MEM_BREAK
...
word write_mem( z80info *z80, word addr, byte val ) {
#ifdef MEM_BREAK
    if ( z80->membrk[ addr ] & M_BREAKPOINT ) {
        fprintf( stderr, "\r\nBreak at 0x%X\r\n", addr );
