...
static void closeall( z80info *z80 ) {
    int i;
...
void warmboot( z80info *z80 ) {
    ...
    for ( i = 0; i < NUMHDISCS; i++ ) {
        ...
        SETMEM( HDPBASE + DPHSIZE * i + 14, ( HALVBASE + HALVSIZE * i ) & 0xFF );
        SETMEM( HDPBASE + DPHSIZE * i + 15, ( HALVBASE + HALVSIZE * i ) >> 8 );
    }
...
#else
        SETMEM( DPBASE + DPHSIZE * i + 12, ( CVBASE + CVSIZE * i ) & 0xFF );
        SETMEM( DPBASE + DPHSIZE * i + 13, ( CVBASE + CVSIZE * i ) >> 8 );
#endif
        SETMEM( DPBASE + DPHSIZE * i + 14, ( ALVBASE + ALVSIZE * i ) & 0xFF );
        SETMEM( DPBASE + DPHSIZE * i + 15, ( ALVBASE + ALVSIZE * i ) >> 8 );
    }
...
static void boot( z80info *z80 ) {
    z80->drive = 0;
    warmboot( z80 );
}

void sysreset( z80info *z80 ) { boot( z80 ); }

static void consstat( z80info *z80 ) { input( z80, 0x01, 0x01, &A ); }

static void consin( z80info *z80 ) {
...
static void consout( z80info *z80 ) { output( z80, 0x00, 0x00, C & 0x7F ); }

/* list character in C */
static void list( z80info *z80 ) {
    static FILE *fp = NULL;
...
static void punch( z80info *z80 ) { (void)z80; }

/* return reader char in A, ^Z is EOF */
static void reader( z80info *z80 ) {
    (void)z80;
    A = CNTL( 'Z' );
}
...
static void settrack( z80info *z80 ) {
    int tracks = ( z80->drive < NUMHDISCS ) ? HDTRACKSPERDISC : TRACKSPERDISC;
...
static void setsector( z80info *z80 ) {
    int sectors = ( z80->drive < NUMHDISCS ) ? HDSECTORSPERTRACK : SECTORSPERTRACK;
...
static void secttran( z80info *z80 ) {
    if ( z80->drive < NUMHDISCS ) {
...
static void liststat( z80info *z80 ) { A = 0xFF; }
...
/* DE points to a CP/M FCB.
   On return, A contains 0 if all went well, 0xFF otherwise.
 */
static void closeunix( z80info *z80 ) {
    int fd_no;
...
void bios( z80info *z80, unsigned int fn ) {
