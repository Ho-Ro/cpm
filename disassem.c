...
static char *rr_names[] = { "B", "C", "D", "E", "H", "L", "(HL)", "A" };

static char *dd_names[] = { "BC", "DE", "HL", "SP" };

static char *qq_names[] = { "BC", "DE", "HL", "AF" };

static char *cc_names[] = { "NZ", "Z", "NC", "C", "PO", "PE", "P", "M" };

static char *op_names[] = {
    "ADD   A,", "ADC   A,", "SUB   ", "SBC   A,", "AND   ", "XOR   ", "OR    ", "CP    ",
};

static char *jr_op_names[] = { 0, 0, "DJNZ  ", "JR    ", "JR    NZ,", "JR    Z,", "JR    NC,", "JR    C," };

static char *log_op_names[] = { "RLCA", "RRCA", "RLA", "RRA", "DAA", "CPL", "SCF", "CCF" };

static char *shf_op_names[] = { "RLC   ", "RRC   ", "RL    ", "RR    ", "SLA   ", "SRA   ", "SLL?  ", "SRL   " };
...
static char *rep_op_names[] = { "LDI",  "CPI",  "INI",  "OUTI", "LDD",  "CPD",  "IND",  "OUTD",
...
static void put_addr( z80info *z80, int loc ) {
...
static void put_digit( byte digit ) {
...
static void put_reg( z80info *z80, int reg_num, int loc ) {
    int d;
...
static void put_dd_reg( int reg_num ) {
    if ( index_reg ) {
        if ( reg_num == 2 )
            put_str( index_reg );
        else if ( reg_num == 3 )
...
static void put_qq_reg( int reg_num ) {
    if ( index_reg ) {
        if ( reg_num == 2 )
            put_str( index_reg );
        else
...
int disassemlen( void ) { return str_length; }
    ...
    case 3:
        ...
        if ( z80->mem[ loc ] == 0xED ) {
            ...
            case 2:
                ...
                put_str( rep_op_names[ ( ( z80->mem[ loc ] >> 1 ) & 0xC ) | ( z80->mem[ loc ] & 3 ) ] );
                break;
